...
a binding consumed by ``b12x.attention.paged.api.paged_attention_forward``.
"""
...
class B12XPagedAttentionScratchCaps:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "kv_quant", str(self.kv_quant or "none"))
...
class B12XPagedAttentionScratch:
    ...
    def bind(
        ...
        *,
        ...
        skip_replay_metadata_update: bool = False,
    ) -> "B12XPagedAttentionBinding":
        return build_paged_attention_binding(
            scratch=self,
            skip_replay_metadata_update=skip_replay_metadata_update,
            q=q,
            k_cache=k_cache,
            v_cache=v_cache,
            output=output,
    ...
    def prepare(
        ...
        *,
        ...
        # Phase 4 meta-once: layers of one family (same shared scratch storage,
        # identical plans/geometry, same runtime page_table/cache_seqlens)
        # write byte-identical replay metadata into the SAME buffers. The
        # family's OWNER layer performs the copy + runtime update each step;
        # followers pass skip_replay_metadata_update=True and only re-bind
        # runtime tensor refs. Only honored on the graph-replay fast paths --
        # a full re-plan below always rebuilds its own metadata.
        with record_function(f"paged_scratch.prepare.{self.mode}"):
            ...
            if self.use_cuda_graph and torch.cuda.is_current_stream_capturing():
                ...
                if skip_replay_metadata_update:
                ...
                if (
                    ...
                    self._decode_graph_metadata_captured_in_graph = True
                elif self._is_msa_verify_graph_replay_plan():
                    self._update_msa_verify_graph_replay_metadata_from_runtime()
                    self._decode_graph_metadata_captured_in_graph = True
                elif (
                    self.mode in ("extend", "verify")
                    and self._prefill_graph_max_q_tiles_per_req is not None
                ):
            ...
            if (
                self.use_cuda_graph
                and self._is_msa_verify_graph_replay_plan()
                and self._plan is not None
            ):
                if int(window_left) != int(self._plan.window_left):
                    raise ValueError(
                        "MSA verify graph replay scratch was prepared with "
                        f"window_left={self._plan.window_left}, got "
                        f"window_left={int(window_left)}"
                    )
                self._bind_runtime_metadata(page_table, cache_seqlens, cu_seqlens_q)
            ...
            if (
                ...
                if int(window_left) != int(self._plan.window_left):
                    raise ValueError(
                        "prefill graph replay scratch was prepared with "
                        f"window_left={self._plan.window_left}, got "
                        f"window_left={int(window_left)}"
                    )
                self._bind_runtime_metadata(page_table, cache_seqlens, cu_seqlens_q)
    ...
    def update_decode_graph_replay_metadata_from_runtime_cache_seqlens(
        ...
        if not self._plan.split_kv:
            ...
            from b12x.attention.paged.graph_replay import update_decode_graph_chunk_metadata
    ...
    def _cache_prefill_graph_replay_shape_from_plan(self) -> None:
        """Re-derive the worst-case grouped (q_tiles x chunks) shape from the
        cached plan. _copy_cached_plan_metadata clears the shape every bind
        (it is decode-oriented), so the prefill/verify update self-heals from
        the plan here -- matching PagedAttentionWorkspace.
        """
        if self._plan is None:
            raise RuntimeError("prefill graph scratch has not been prepared")
        active = [
            (qi, ki)
            for qi, ki, valid in zip(
        ...
        max_q_tiles = max(qi for qi, _ in active) + 1
        max_chunks = max(ki for _, ki in active) + 1
        max_q_rows = max(
            1,
            (
                int(max_q_tiles) * int(self._plan.cta_tile_q)
                + int(self._plan.gqa_group_size)
                - 1
            )
            // int(self._plan.gqa_group_size),
        )
        self._prefill_graph_max_q_tiles_per_req = int(max_q_tiles)
        self._prefill_graph_max_chunks_per_q_tile = int(max_chunks)
        self._prefill_graph_max_q_rows_per_req = int(max_q_rows)

    def _is_msa_verify_graph_replay_plan(self) -> bool:
        return bool(
            self.use_cuda_graph
            and self.mode == "verify"
            and self._plan is not None
            and getattr(self._plan, "msa_block_sparse", False)
            and self._decode_graph_chunk_pages_lut is not None
        )

    def _update_msa_verify_graph_replay_metadata_from_runtime(self) -> None:
        """Per-step graph-safe metadata refresh for MSA block-sparse verify.

        Native verify groups q_len rows per request, but MSA's effective KV span
        is topk-bounded and should be derived from runtime cache_seqlens. This is
        the verify analogue of update_msa_decode_graph_chunk_metadata: compact
        work items are [request, qo_tile, kv_chunk], merge_indptr is per q row,
        and o_indptr remains per request.
        """
        if self._plan is None:
            raise RuntimeError("MSA verify graph scratch has not been prepared")
        if self.cache_seqlens is None or self.cu_seqlens_q is None:
            raise RuntimeError("MSA verify graph scratch is missing cache_seqlens/cu_seqlens_q")
        if self.request_indices is None:
            raise RuntimeError("MSA verify graph scratch is missing request indices")
        if self.qo_tile_indices is None or self.kv_tile_indices is None:
            raise RuntimeError("MSA verify graph scratch is missing tile indices")
        if self.merge_indptr is None or self.o_indptr is None:
            raise RuntimeError("MSA verify graph scratch is missing indptr buffers")
        if self.kv_chunk_size_ptr is None:
            raise RuntimeError("MSA verify graph scratch is missing kv_chunk_size_ptr")
        if self.total_num_rows_ptr is None:
            raise RuntimeError("MSA verify graph scratch is missing total_num_rows_ptr")
        if self.block_valid_mask is None:
            raise RuntimeError("MSA verify graph scratch is missing block_valid_mask")
        if self.kv_window_start_tokens is None:
            raise RuntimeError("MSA verify graph scratch is missing kv_window_start_tokens")
        batch = int(self._plan.page_table_shape[0])
        max_q_rows_per_req = max(int(self._plan.total_q) // max(batch, 1), 1)
        ...
        update_msa_verify_graph_chunk_metadata(
            ...
            max_q_rows_per_req=max_q_rows_per_req,
        )

    def _update_prefill_graph_replay_metadata_from_runtime(self) -> None:
        """Per-step (graph-captured) recompute of the grouped extend/verify work
        items from runtime cache_seqlens + cu_seqlens_q. Eager port of
        PagedAttentionWorkspace._update_prefill_graph_replay_metadata_from_runtime.
        """
        if self._plan is None:
            raise RuntimeError("prefill graph scratch has not been prepared")
        if self.cache_seqlens is None or self.cu_seqlens_q is None:
            raise RuntimeError(
                "prefill graph scratch is missing cache_seqlens/cu_seqlens_q"
            )
        if (
        ...
        from b12x.attention.paged.graph_replay import update_prefill_graph_chunk_metadata
    ...
    def _validate_static_shapes(
        ...
        exp_k = self.head_dim_qk
        exp_v = self.head_dim_vo
        if self.kv_quant == "nvfp4":
            # vLLM-native packed layout: e2m1 (head_dim/2) + e4m3 per-16 block
            # scale (head_dim/16) = head_dim*9/16 B/head, NO padding (72 for 128).
            # The decode cp.async.bulk reads a 16B-rounded superset, so the head
            # stride does not need to be 16B-aligned.
            exp_k = self.head_dim_qk // 2 + self.head_dim_qk // 16
            exp_v = self.head_dim_vo // 2 + self.head_dim_vo // 16
        if int(k_cache.shape[3]) != exp_k:
            raise ValueError("k_cache head_dim does not match the scratch contract")
        if int(v_cache.shape[3]) != exp_v:
...
def _materialize_paged_attention_scratch(
    ...
    *,
    ...
    prefill_graph_max_q_tiles_per_req: int | None = None,
    prefill_graph_max_chunks_per_q_tile: int | None = None,
    prefill_graph_max_q_rows_per_req: int | None = None,
) -> B12XPagedAttentionScratch:
    ...
    return B12XPagedAttentionScratch(
        ...
        _prefill_graph_max_q_tiles_per_req=prefill_graph_max_q_tiles_per_req,
        _prefill_graph_max_chunks_per_q_tile=prefill_graph_max_chunks_per_q_tile,
        _prefill_graph_max_q_rows_per_req=prefill_graph_max_q_rows_per_req,
    )
...
class B12XPagedAttentionScratchPlan:
    ...
    _prefill_graph_max_q_rows_per_req: int | None = None
    _q2k_indices_data_ptr: int | None = None
    _resolved_policy: PagedDecodePolicy | None = None
    ...
    @property
    def resolved_policy(self) -> PagedDecodePolicy | None:
        """Descriptive policy recorded by the graph prepare_* methods."""
        return self._resolved_policy

    def _record_resolved_policy(
        self,
        *,
        mode: str,
        max_batch: int,
        ...
        """Record (and optionally print) the resolved plan policy.

        Purely descriptive: mirrors decisions the planner already made, so this
        never changes behavior. B12X_DEBUG_PAGED_POLICY=1 prints one line.
        """
        policy = PagedDecodePolicy(
            family="msa" if self.caps.msa_block_sparse else "dense",
            mode=mode,
            kv_quant=normalize_kv_quant(self.caps.kv_quant, self.caps.kv_dtype),
            kv_dtype=str(self.caps.kv_dtype),
            page_size=int(self.caps.page_size),
            head_dim_qk=int(self.caps.head_dim_qk),
            head_dim_vo=int(self.caps.head_dim_vo),
            gqa_group_size=int(self.caps.num_q_heads) // max(int(self.caps.num_kv_heads), 1),
            max_batch=int(max_batch),
            max_total_q=int(max_total_q),
            max_page_table_width=int(max_page_table_width),
            cache_page_capacity=int(cache_page_capacity),
            split_kv=bool(split_kv),
            chunk_pages=None if chunk_pages is None else int(chunk_pages),
            max_chunks_per_request=(
                None if max_chunks_per_request is None else int(max_chunks_per_request)
            ),
            graph_ctas_per_sm=None if graph_ctas_per_sm is None else int(graph_ctas_per_sm),
            metadata_strategy=metadata_strategy,
            backend_variant="base",
            use_cuda_graph=bool(self.caps.use_cuda_graph),
        )
        self._resolved_policy = policy
        emit_resolved_policy(policy, site=site)
    ...
    def prepare_verify_graph_replay_state(
        self,
        *,
        batch: int,
        q_len: int,
        ...
        """Graph-mode prepare for block-sparse spec-decode VERIFY (q_len>1).

        Mirrors the block-sparse branch of prepare_decode_graph_replay_state but
        with mode="verify": the worst-case plan groups q_len queries per request
        (cu_seqlens_q = arange(batch+1)*q_len; page_table/cache_seqlens are per
        request). Block-sparse KV is bounded by topk, so the plan structure is
        context-independent -- the cached plan metadata stays valid as
        cache_seqlens grows, and the capture path (class-1 prepare) refreshes
        page_table/cache_seqlens WITHOUT a decode chunk-LUT update (that update
        is gated on mode=="decode"). The kernel selects each qo-tile's own q2k
        row (msa_q_row_idx = q_start + qo_tile_idx). Validated captured + mutated
        in tools/nvfp4-kv/msa_verify_test.py.
        """
        if not self.caps.use_cuda_graph:
            raise RuntimeError(
                "prepare_verify_graph_replay_state is only valid for graph-mode "
                "paged scratch plans"
            )
        if self.caps.mode != "verify":
            raise RuntimeError(
                "prepare_verify_graph_replay_state is only valid for verify plans"
            )
        if not self.caps.msa_block_sparse:
            raise RuntimeError(
                "prepare_verify_graph_replay_state is only valid for MSA "
                "block-sparse plans (full-attention verify is unsupported)"
            )
        if batch <= 0 or q_len < 1:
            raise ValueError("batch must be positive and q_len >= 1")
        ...
        if self.caps.page_size not in (64, 128):
            raise ValueError("MSA block-sparse verify requires page_size=64 or 128")
        if self.caps.head_dim_qk != 128 or self.caps.head_dim_vo != 128:
            raise ValueError("MSA block-sparse verify requires head_dim_qk=head_dim_vo=128")
        if self.caps.num_q_heads // self.caps.num_kv_heads != 16:
            raise ValueError("MSA block-sparse verify requires gqa_group_size=16")
        total_q_capacity = int(batch) * int(q_len)
        max_page_ids = torch.arange(
            max_page_table_width, dtype=torch.int32, device=self.caps.device
        )
        # page table + cache_seqlens are PER REQUEST (verify groups q_len queries
        # of a request via cu_seqlens_q; the kernel replicates per qo-tile).
        max_page_table = (
        ...
        max_cu_seqlens_q = (
            torch.arange(0, batch + 1, dtype=torch.int32, device=self.caps.device)
            * int(q_len)
        )
        plan = create_paged_plan(
            self._plan_q[:total_q_capacity],
            self._plan_k_cache,
            self._plan_v_cache,
            max_page_table,
            max_cache_seqlens,
            max_cu_seqlens_q,
            mode="verify",
            kv_quant=self.caps.kv_quant,
            fixed_split_size=int(fixed_split_size),
            force_split_kv=True,
            window_left=-1,
            enable_cuda_graph=True,
            graph_chunk_policy=False,
            max_batch_size_if_split=total_q_capacity * 32,
        ...
        # dummy LUT: block-sparse KV is bounded so there is no per-step chunk
        # replay (the class-1 capture path skips the LUT update when mode!=decode).
        ...
        self._decode_graph_max_chunks_per_req = max(
            ...
            for idx in range(len(plan.o_indptr) - 1)
        )
        self._use_regular_decode_graph_replay = False
        self._record_resolved_policy(
            mode="verify",
            max_batch=batch,
            max_total_q=total_q_capacity,
            max_page_table_width=max_page_table_width,
            cache_page_capacity=max_cache_page_count,
            split_kv=True,
            chunk_pages=None if int(fixed_split_size) <= 0 else int(fixed_split_size),
            max_chunks_per_request=self._decode_graph_max_chunks_per_req,
            graph_ctas_per_sm=None,
            metadata_strategy="captured-update",
            site="paged_scratch.verify_graph",
        )
        return self
    ...
    def prepare_prefill_graph_replay_state(
        ...
        *,
        ...
        """Graph-mode prepare for FULL-attention (non-block-sparse) spec-decode
        VERIFY / extend. Builds a worst-case split-kv plan whose grouped work
        items (q_len qo-tiles per request sharing the request's KV split,
        cu_seqlens_q grouped) are recomputed PER REPLAY from runtime
        cache_seqlens by update_prefill_graph_chunk_metadata -- the per-step
        adaptive path. Eager port of
        PagedAttentionWorkspace.prepare_prefill_graph_replay_state (the sglang
        extend/verify graph-replay machinery).
        """
        if not self.caps.use_cuda_graph:
            raise RuntimeError(
                "prepare_prefill_graph_replay_state is only valid for graph-mode plans"
            )
        if self.caps.mode not in ("extend", "verify"):
            raise RuntimeError(
                "prepare_prefill_graph_replay_state is only valid for extend/verify plans"
            )
        if self.caps.msa_block_sparse:
            raise RuntimeError(
                "MSA block-sparse verify uses prepare_verify_graph_replay_state"
            )
        if batch <= 0:
        ...
        if max_page_table_width <= 0:
            raise ValueError("max_page_table_width must be positive")
        if window_left < -1:
            raise ValueError("window_left must be -1 or a non-negative token count")
        ...
        self._cache_prefill_graph_replay_shape_from_plan(plan)
        ...
        self._record_resolved_policy(
            ...
            split_kv=True,
            chunk_pages=None,
            max_chunks_per_request=self._prefill_graph_max_chunks_per_q_tile,
            graph_ctas_per_sm=None,
            metadata_strategy="captured-update",
            site="paged_scratch.prefill_graph",
        )
        return self

    def _cache_prefill_graph_replay_shape_from_plan(self, plan: PagedPlan) -> None:
        active = [
            (ri, qi, ki)
            for ri, qi, ki, valid in zip(
                plan.request_indices,
                plan.qo_tile_indices,
        ...
        if not active:
            raise RuntimeError("prefill graph replay plan contains no active work")
        max_q_tiles = max(qi for _, qi, _ in active) + 1
        max_chunks = max(ki for _, _, ki in active) + 1
        max_q_rows = max(
            1,
            (
                int(max_q_tiles) * int(plan.cta_tile_q)
        ...
        *,
        ...
        if self.caps.msa_block_sparse:
            ...
            self._record_resolved_policy(
                ...
                graph_ctas_per_sm=None,
                metadata_strategy="captured-update",
                site="paged_scratch.decode_graph",
            )
            return self
        ...
        if force_split_kv:
            # Split capacity plans must be built at the FULL page span: the
            # summarize() worst case maximizes the CHUNK COUNT (smallest
            # chunk_pages, e.g. 1 page), but the runtime LUT update can select
            # chunk_pages up to LUT[max_pages] -- the static plan must cover
            # the largest per-chunk extent, not the largest chunk count.
            capacity_cache_seqlen = int(max_cache_page_count) * self.caps.page_size
        if window_left >= 0:
        ...
        # Dense decode: unsplit by default. With force_split_kv=True the
        # capacity plan splits each request's KV across the chunk-LUT work
        # items (partial rows + merge every step -- the same rails as MSA
        # decode); the runtime LUT metadata update then re-chunks per step.
        plan = create_paged_plan(
            self._plan_q[:batch],
            self._plan_k_cache,
            self._plan_v_cache,
        ...
        self._record_resolved_policy(
            ...
            max_page_table_width=max_page_table_width,
            cache_page_capacity=max_cache_page_count,
            split_kv=bool(force_split_kv),
            chunk_pages=None,
            max_chunks_per_request=int(max_chunks_per_req),
    ...
    def bind(
        ...
        *,
        ...
        scratch_views = _materialize_paged_attention_scratch(
            ...
            prefill_graph_max_q_tiles_per_req=self._prefill_graph_max_q_tiles_per_req,
            prefill_graph_max_chunks_per_q_tile=self._prefill_graph_max_chunks_per_q_tile,
            prefill_graph_max_q_rows_per_req=self._prefill_graph_max_q_rows_per_req,
        )
...
def plan_paged_attention_scratch(
    ...
    k_store_last = caps.head_dim_qk
    v_store_last = caps.head_dim_vo
    if caps.kv_quant == "nvfp4":
        # vLLM-native packed storage: e2m1 (hd/2) + e4m3 scale (hd/16), NO pad
        # (72 for 128). Decode reads a 16B-rounded superset (no aligned stride).
        k_store_last = caps.head_dim_qk // 2 + caps.head_dim_qk // 16
        v_store_last = caps.head_dim_vo // 2 + caps.head_dim_vo // 16
    plan_k_cache = _shape_only_cuda_tensor(
        (caps.num_cache_pages, caps.page_size, caps.num_kv_heads, k_store_last),
        dtype=caps.kv_dtype,
        device=caps.device,
    )
    plan_v_cache = _shape_only_cuda_tensor(
        (caps.num_cache_pages, caps.page_size, caps.num_kv_heads, v_store_last),
        dtype=caps.kv_dtype,
        device=caps.device,
    )
    planner_budget = PagedPlanBudget(
