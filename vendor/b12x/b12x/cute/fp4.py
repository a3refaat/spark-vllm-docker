...
@dsl_user_op
def mxf4nvf4_mma_m16n8k64_f32_e2m1(
    d0: Float32,
    d1: Float32,
    d2: Float32,
    d3: Float32,
    ...
    *,
    ...
    """Warp MMA helper for SM120 native NVFP4 block-scaled `m16n8k64` E2M1/E2M1.

    The ONLY NVFP4 block-scaled MMA on SM12x: ``kind::mxf4nvf4`` with
    ``scale_vec::4X`` (4 ue4m3 scale factors spanning K=64, i.e. one per
    16-element micro-block) and ``.ue4m3`` scale type.  Operand structure is
    identical to ``mxfp8_mma_m16n8k32_f32_e4m3``: A = 4xu32 (16x64 e2m1, 8
    nibbles/reg), B = 2xu32 (64x8 e2m1), D/C = 4xf32, and one packed scale-data
    register per operand with ``{byte-id, thread-id}`` selectors (CUTLASS
    convention tid=0).  ``sfa``/``sfb`` each pack the 4 ue4m3 micro-block scales
    in one u32.  d{0..3} are the accumulator IN-OUT (C on input, D on output).
    """
    i16_ty = cutlass._mlir.ir.IntegerType.get_signless(16)
    bid_a_i16 = cutlass._mlir.ir.Operation.create(
        "llvm.mlir.constant",
