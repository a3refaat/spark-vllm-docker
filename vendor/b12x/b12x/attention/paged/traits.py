...
def select_paged_forward_traits_from_plan(
    ...
    *,
    ...
    # nvfp4 KV is loaded as packed e2m1 + an e4m3 per-16 block scale, then
    # EXPANDED to bf16 in SMEM and fed through the bf16 MMA path. So the MMA
    # operand tile is bf16 -> select bf16 traits. The packed-cache load/expand
    # is gated separately by the kernel's kv_is_nvfp4 flag (from plan.kv_quant).
    kv_dtype = torch.bfloat16 if plan.kv_quant == "nvfp4" else plan.kv_dtype
