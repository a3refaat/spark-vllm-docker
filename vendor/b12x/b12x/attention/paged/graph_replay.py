...
@triton.jit
def update_msa_verify_graph_metadata_fused_triton(
    cache_seqlens_ptr,
    cu_seqlens_q_ptr,
    merge_indptr_ptr,
    o_indptr_ptr,
    ...
    q_start = tl.load(cu_seqlens_q_ptr + batch_offsets, mask=batch_mask, other=0).to(
        tl.int32
    )
    q_end = tl.load(cu_seqlens_q_ptr + batch_offsets + 1, mask=batch_mask, other=0).to(
        tl.int32
    )
    q_len = tl.maximum(q_end - q_start, 0)
    active = batch_mask & (cache_len > 0) & (q_len > 0)
    ...
    num_chunks = tl.maximum((selected_tokens + kv_chunk_size - 1) // kv_chunk_size, 1)
    num_chunks = tl.where(active, num_chunks, 0)
    partial_rows = q_len * num_chunks
    prefix = tl.cumsum(tl.where(batch_mask, partial_rows, 0), 0)
    ...
    tl.store(kv_chunk_size_ptr, kv_chunk_size)
    tl.store(total_num_rows_ptr, tl.load(cu_seqlens_q_ptr + BATCH).to(tl.int32))
...
@triton.jit
def update_msa_verify_graph_row_indptr_triton(
    cu_seqlens_q_ptr,
    o_indptr_ptr,
    merge_indptr_ptr,
    BATCH: tl.constexpr,
    ...
    num_chunks = tl.maximum((request_partial_end - request_partial_start) // safe_q_len, 0)
...
@triton.jit
def update_msa_verify_graph_compact_work_metadata_triton(
    cu_seqlens_q_ptr,
    o_indptr_ptr,
    request_indices_ptr,
    qo_tile_indices_ptr,
    ...
    BLOCK_QCHUNKS: tl.constexpr,
):
    req_idx = tl.program_id(axis=0)
    chunk_block_idx = tl.program_id(axis=1)
    qchunk = chunk_block_idx * BLOCK_QCHUNKS + tl.arange(0, BLOCK_QCHUNKS)
    q_tile_idx = qchunk // MAX_CHUNKS_PER_Q_TILE
    kv_tile_idx = qchunk - q_tile_idx * MAX_CHUNKS_PER_Q_TILE
    ...
    active = (req_idx < BATCH) & (q_tile_idx < q_len) & (kv_tile_idx < num_chunks)
    work_offsets = request_partial_start + q_tile_idx * num_chunks + kv_tile_idx
    work_mask = active & (work_offsets < work_items_capacity)
    valid_mask = active & (work_offsets < block_valid_capacity)
    tl.store(request_indices_ptr + work_offsets, req_idx, mask=work_mask)
    tl.store(qo_tile_indices_ptr + work_offsets, q_tile_idx.to(tl.int32), mask=work_mask)
    tl.store(kv_tile_indices_ptr + work_offsets, kv_tile_idx.to(tl.int32), mask=work_mask)
    tl.store(block_valid_mask_ptr + work_offsets, 1, mask=valid_mask)
...
def update_msa_verify_graph_chunk_metadata(
    *,
    cache_seqlens: torch.Tensor,
    cu_seqlens_q: torch.Tensor,
    request_indices: torch.Tensor,
    ...
    if merge_indptr.device != device or o_indptr.device != device:
        raise ValueError("indptr buffers and cache_seqlens must be on the same device")
    if block_valid_mask.device != device or kv_chunk_size_ptr.device != device:
        raise ValueError("MSA verify graph buffers and cache_seqlens must be on the same device")
    if kv_window_start_tokens.device != device or total_num_rows_ptr.device != device:
        raise ValueError("MSA verify scalar buffers and cache_seqlens must be on the same device")
    if page_size not in (64, 128):
        raise ValueError("MSA verify graph replay requires page_size=64 or page_size=128")
    kv_chunk_size = int(kv_chunk_size)
    if kv_chunk_size <= 0 or kv_chunk_size % 64 != 0:
        raise ValueError("MSA verify graph kv_chunk_size must be a positive multiple of 64 tokens")

    bs = int(cache_seqlens.shape[0])
    if bs <= 0:
        raise ValueError("MSA verify graph replay requires bs > 0")
    if int(cu_seqlens_q.shape[0]) < bs + 1:
        raise RuntimeError("MSA verify graph cu_seqlens_q is smaller than batch + 1")
    work_items_capacity = int(request_indices.shape[0])
    block_valid_capacity = int(block_valid_mask.shape[0])
    if int(qo_tile_indices.shape[0]) != work_items_capacity or int(kv_tile_indices.shape[0]) != work_items_capacity:
        raise RuntimeError("MSA verify graph tile index buffers must match request_indices shape")
    max_q_rows_per_req = max(int(max_q_rows_per_req), 1)
    denom = bs * max_q_rows_per_req
    if denom <= 0 or work_items_capacity < denom:
        raise RuntimeError("MSA verify graph work capacity is too small for batch*q_len")
    max_chunks_per_q_tile = work_items_capacity // denom
    if max_chunks_per_q_tile <= 0:
        raise RuntimeError("MSA verify graph must allocate at least one chunk per query row")
    max_required_chunks = triton.cdiv(16 * 128, kv_chunk_size)
    if max_chunks_per_q_tile < max_required_chunks:
        raise RuntimeError(
            "MSA verify graph work capacity is too small for the chunk size"
        )
    if int(o_indptr.shape[0]) < bs + 1:
        raise RuntimeError("MSA verify graph o_indptr is smaller than batch + 1")
    if int(merge_indptr.shape[0]) < bs * max_q_rows_per_req + 1:
        raise RuntimeError("MSA verify graph merge_indptr is smaller than total_q + 1")
    if int(kv_window_start_tokens.shape[0]) < bs:
        raise RuntimeError("MSA verify graph kv_window_start_tokens is smaller than batch")

    update_msa_verify_graph_metadata_fused_triton[(1,)](
        cache_seqlens,
        cu_seqlens_q,
        merge_indptr,
        o_indptr,
    ...
    update_msa_verify_graph_row_indptr_triton[
        (bs, triton.cdiv(max_q_rows_per_req, _PREFILL_BLOCK_ROWS))
    ](
        cu_seqlens_q,
        o_indptr,
        merge_indptr,
    ...
    update_msa_verify_graph_compact_work_metadata_triton[
        (bs, triton.cdiv(max_q_rows_per_req * max_chunks_per_q_tile, _DECODE_BLOCK_CHUNKS))
    ](
        cu_seqlens_q,
        o_indptr,
        request_indices,
        ...
        BATCH=bs,
        MAX_Q_ROWS_PER_REQ=max_q_rows_per_req,
        MAX_CHUNKS_PER_Q_TILE=max_chunks_per_q_tile,
        BLOCK_QCHUNKS=_DECODE_BLOCK_CHUNKS,
    )
