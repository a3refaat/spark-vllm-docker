    ...
    *,
    ...
    if kv_quant == "nvfp4":
        if k_cache.dtype != torch.uint8:
            raise TypeError(f"nvfp4 paged kv cache must be uint8 e2m1, got {k_cache.dtype}")
    elif k_cache.dtype not in (torch.float16, torch.bfloat16, _FP8_KV_DTYPE):
    ...
    if kv_quant == "nvfp4":
        # Interleaved nvfp4 layout: stored bytes per (token, head) =
        # head_dim/2 (e2m1) + head_dim/16 (e4m3 block scale). The K head dim
        # must equal Q's; recover V's logical head dim from the same packing
        # ratio (k and v share the layout).
        head_dim_vo = head_dim_vo * head_dim_qk // head_dim_k
    ...
    if msa_block_sparse:
        if mode not in ("decode", "extend", "verify"):
            raise ValueError("MSA block-sparse paged attention currently supports decode, extend, and verify plans only")
        # verify: cta_tile_q == group_size so each qo-tile is one query; the
        # kernel selects KV blocks from that query's own q2k row
        # (msa_q_row_idx = q_start + qo_tile_idx). Validated bit-exact vs ref
        ...
        if not (kv_quant == "nvfp4" and k_cache.dtype == torch.uint8) and k_cache.dtype not in (
            torch.bfloat16,
            _FP8_KV_DTYPE,
        ):
            raise TypeError(
                "MSA block-sparse paged attention supports bf16, fp8, or nvfp4 KV only"
            )
        if (
            mode == "extend"
    ...
    if mode == "decode" and force_split_kv and not msa_block_sparse:
        # Dense decode split-kv (Phase 2): ride the same partial+merge rails
        # as MSA decode / dense verify. The merge kernel specializes
        # bf16/fp16/fp32 at head_dim 128 and 256 (paged/api.py merge_bdy
        # selection); reject anything it cannot merge. Unsplit stays the
        # default -- force_split_kv is never inferred True for dense decode.
        if q.dtype not in (torch.float16, torch.bfloat16, torch.float32) or head_dim_vo not in (128, 256):
            raise ValueError(
                "dense decode split-kv requires a merge-supported output "
                f"(dtype={q.dtype}, head_dim_vo={head_dim_vo})"
            )
    ...
    if (
        ...
        print(
            "# b12x_paged_policy"
            f" family={'msa' if msa_block_sparse else 'dense'}"
            f" kv_quant={kv_quant}"
            f" mode={mode}"
            f" batch={batch}"
