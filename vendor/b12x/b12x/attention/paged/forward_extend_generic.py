...
def _expand_nvfp4_interleaved_to_bf16_planes(
    buf_base: Int32,
    ...
    """Expand a row-major SMEM buffer of packed nvfp4 records -> bf16 planes (where
    the stock bf16 MMA reads). Each record is [e2m1 head/2][e4m3 scale head/16][pad]
    with stride `row_stride` (16B-padded). Per u32 (4 contiguous e2m1 bytes = 8
    channels = one 128b granule): fp4_decode_4bytes -> f16x2_to_f32x2 (exact)
    -> * fp8_e4m3_to_f32(per-16 e4m3 scale) -> pack -> st.shared.v4 at
    _permuted_offset_128b(row, vec, upcast_stride_plane). Bit-exact vs ref_nv.
    """
    bytes_per_row = head_dim // 2          # 64 e2m1 bytes per token
    ...
    while i < Int32(total_u32):
        ...
        qj = i % Int32(u32_per_row)
        base = buf_base + row * Int32(row_stride)
        packed = ld_shared_u32(base + qj * Int32(4))
        blk = qj // Int32(2)               # per-16 scale block (== ch // 16)
        sc_word = ld_shared_u32(base + Int32(bytes_per_row) + (blk & Int32(0x7FFFFFFC)))
...
class PagedForwardKernel:
    def __init__(
        ...
        *,
        ...
        # nvfp4: cache tensor is uint8 (packed e2m1 + e4m3 scale, 16B-padded head
        # stride); the SMEM tile / MMA operand stays bf16 (dtype_kv_storage). The
        # packed cache is loaded by a cp.async.bulk gather + in-SMEM expand.
        self.kv_is_nvfp4 = bool(kv_is_nvfp4)
        self.kv_cache_elem_dtype = cutlass.Uint8 if bool(kv_is_nvfp4) else dtype_kv_storage
        # per-head packed record width (e2m1 + e4m3 scale) padded to a 16B-aligned
        # stride so the cp.async gmem src is 16B-aligned (head_stride % 16 == 0).
        self.nvfp4_store_w = ((traits.head_dim_qk // 2 + traits.head_dim_qk // 16 + 15) // 16) * 16
    ...
    def _get_fp8_planewords_dump_storage_cls(self):
        class SharedStorage:
            pass

        SharedStorage.__annotations__ = {
            "mbar_ptr_V": cute.struct.MemRange[cutlass.Int64, 1],
            "payload": cute.struct.Align[
                cute.struct.MemRange[
                    cutlass.Uint8,
                    int(self.stage_tile_rows * self.traits.head_dim_vo * (self.dtype_kv_storage.width // 8)),
                ],
                1024,
            ],
        }
        ...
        for tile_iter in cutlass.range_constexpr(self.traits.num_mma_kv * 4 // self.traits.num_warps_q):
            ...
            for vec_iter in cutlass.range_constexpr((row_bytes + 127) // 128):
                ...
                if const_expr(self.kv_is_nvfp4):
                    # Inline nvfp4 expand: one 128b plane granule = 8 channels =
                    # 4 packed e2m1 bytes + one per-16 e4m3 scale. Read them from
                    # the uint8 cache (16B-padded record => 4B-aligned e2m1) and
                    # write the expanded bf16 granule where the bf16 MMA reads.
                    # mCacheBytes strides are uint8 byte strides (cache_elem_bytes=1).
                    dst_u32 = shared_ptr_to_u32(sStageBytes.iterator + dst_byte_idx)
                    if vec_valid:
                        packed = ld_global_nc_u32(
                            get_ptr_as_int64(mCacheBytes, row_byte_base + vec_idx * Int32(4))
                        )
                        blk = vec_idx // Int32(2)
                        sc_word = ld_global_nc_u32(
                            get_ptr_as_int64(
                                mCacheBytes,
                                row_byte_base + Int32(row_bytes // 4) + (blk & Int32(0x7FFFFFFC)),
                            )
                        )
                        sc_byte = (sc_word >> ((blk & Int32(3)) * Int32(8))) & Uint32(0xFF)
                        sf = fp8_e4m3_to_f32(sc_byte)
                        ...
                        f2a, f2b = f16x2_to_f32x2(h45); u2 = pack_f32x2_to_bfloat2(f2a * sf, f2b * sf)
                        f3a, f3b = f16x2_to_f32x2(h67); u3 = pack_f32x2_to_bfloat2(f3a * sf, f3b * sf)
                        st_shared_v4_u32(dst_u32, u0, u1, u2, u3)
                    else:
                        st_shared_v4_u32(dst_u32, Uint32(0), Uint32(0), Uint32(0), Uint32(0))
                elif const_expr(fill_zero):
    ...
    @cute.jit
    def _async_copy_paged_nvfp4_row(
        self,
        mCacheBytes: cute.Tensor,
        mPageTable: cute.Tensor,
        request_idx,
        ...
        """nvfp4 cp.async gather: copy each token's packed record (e2m1 + e4m3 scale,
        padded to a 16B-aligned `store_w`) ROW-MAJOR into the SMEM gather buffer.
        The consumer expands it into the bf16 planes after cp_async_wait_group +
        sync. Mirrors _async_copy_paged_tile_permuted_128b's row assignment."""
        page_size = Int32(self.page_size)
        lane_row = lane // 8
        lane_col = lane % 8
        for tile_iter in cutlass.range_constexpr(self.traits.num_mma_kv * 4 // self.traits.num_warps_q):
            ...
            for vec_iter in cutlass.range_constexpr((store_w + 127) // 128):
                vec_idx = Int32(lane_col + vec_iter * 8)
                src_byte_idx = row_byte_base + vec_idx * Int32(16)
                dst = buf_base_u32 + stage_byte_offset + row_idx * Int32(store_w) + vec_idx * Int32(16)
                vec_valid = row_valid and (vec_idx * Int32(16) < Int32(store_w))
                _cp_async_load_128b_pred(
                    dst,
                    get_ptr_as_int64(mCacheBytes, src_byte_idx),
                    Int32(vec_valid),
    ...
    def __call__(
        ...
        if const_expr(
            self.debug_dump_paged_kv_planewords
            and self.kv_is_fp8
            and self.use_paged_kv_tma_fp8_raw_issue
            and not self.use_paged_k_tma
            and self.use_paged_v_tma
        ):
            DumpStorage = self._get_fp8_planewords_dump_storage_cls()
            self.kernel_fp8_v_planewords_dump(
                mPageTable,
                mO,
                mVTmaDescPtrs,
            ).launch(
                grid=(1, 1, 1),
                block=[32, 1, 1],
                smem=DumpStorage.size_in_bytes(),
                stream=stream,
            )
            return
    ...
    @cute.kernel
    def kernel_fp8_v_planewords_dump(
        self,
        mPageTable: cute.Tensor,
        mO: cute.Tensor,
        mVTmaDescPtrs: cute.Tensor,
        ...
        SharedStorage = self._get_fp8_planewords_dump_storage_cls()
        smem = cutlass.utils.SmemAllocator()
        storage = smem.allocate(SharedStorage)
        mbar_ptr_V = storage.mbar_ptr_V.data_ptr()
        sVStageBytes = storage.payload.get_tensor(
            cute.make_layout((self.stage_tile_rows * self.traits.head_dim_vo * (self.dtype_kv_storage.width // 8),), stride=(1,))
        )
        if lane == 0:
        ...
        self._issue_paged_kv_tma_copy_2planes_fp8_raw(
            ...
            sVStageBytes,
            Int32(0),
            self.stage_tile_rows * self.kv_tma_plane_head_dim * (self.dtype_kv_storage.width // 8),
            producer_state,
            mbar_ptr_V,
    ...
    def kernel(
        ...
        # cache element size in bytes for GMEM address math: nvfp4 cache is uint8
        # (packed), so its strides are already byte strides (x1); bf16/fp8 use the
        # storage element size.
        cache_elem_bytes = 1 if self.kv_is_nvfp4 else kv_storage_bytes
        k_page_stride_bytes = mKCache.stride[0] * cache_elem_bytes
        k_token_stride_bytes = mKCache.stride[1] * cache_elem_bytes
        k_head_stride_bytes = mKCache.stride[2] * cache_elem_bytes
        v_page_stride_bytes = mVCache.stride[0] * cache_elem_bytes
        v_token_stride_bytes = mVCache.stride[1] * cache_elem_bytes
        v_head_stride_bytes = mVCache.stride[2] * cache_elem_bytes
