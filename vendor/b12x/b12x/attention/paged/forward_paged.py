...
from b12x.cute.fp4 import get_ptr_as_int64, shared_ptr_to_u32, elem_pointer
...
@cute.jit
def _expand_nvfp4_to_bf16_planes(
    s_e2m1_addr: Int32,
    s_scale_addr: Int32,
    plane0_base: Int32,
    plane1_base: Int32,
    upcast_stride_plane: cutlass.Constexpr,
    tid: Int32,
    num_threads: cutlass.Constexpr,
    stage_tile_rows: cutlass.Constexpr,
    head_dim: cutlass.Constexpr,
    e2m1_row_bytes: cutlass.Constexpr,
    kv_head_idx: Int32,
):
    """Expand one staged tile of packed nvfp4 -> bf16 plane SMEM, writing EXACTLY
    where the stock bf16 ldmatrix reads (the expand REPLACES the TMA, so it must
    match the MMA read addressing, not the TMA write layout).

    Per u32 (4 contiguous e2m1 bytes) = 8 channels = one 16-byte / 128-bit granule:
      fp4_decode_4bytes -> 4 f16x2 (even->low order) -> f16x2_to_f32x2 (exact)
      -> * fp8_e4m3_to_f32(per-16 e4m3 scale) -> pack_f32x2_to_bfloat2 -> a single
      st.shared.v4.u32 at _permuted_offset_128b(row, vec, upcast_stride_plane).
    Numerics validated bit-exact vs ref_nv in tools/nvfp4-kv/cute_expand_validate.py.

      s_e2m1_addr  : stage-adjusted u32 SMEM addr of e2m1 buf [rows, head_dim//2] u8.
      s_scale_addr : stage-adjusted u32 SMEM addr of e4m3 scale buf [rows, head_dim//16] u8.
      plane0/1_base: stage-adjusted bf16 plane base addrs (same as the bf16 QK MMA).
      upcast_stride_plane: bf16 plane row stride in 128b granules (== tc_upcast_stride_plane).
    """
    bytes_per_row = head_dim // 2          # 64 for head_dim 128
    u32_per_row = bytes_per_row // 4       # 16 (each u32 = 8 channels = 1 granule)
    scales_per_row = head_dim // 16        # 8
    granules_per_plane = 64 // 8           # 8
    total_u32 = stage_tile_rows * u32_per_row
    # ld_global producer packs the native head/2 e2m1 contiguous at row*e2m1_row_bytes
    # (64 B/row, NO round-down) -> head_offset is always 0.
    head_offset = Int32(0)
    i = tid
    while i < Int32(total_u32):
        row = i // Int32(u32_per_row)
        qj = i % Int32(u32_per_row)        # u32 index within the row (0..15)
        packed = ld_shared_u32(s_e2m1_addr + row * Int32(e2m1_row_bytes) + head_offset + qj * Int32(4))
        blk = qj // Int32(2)               # per-16 scale block (== ch // 16)
        sc_word = ld_shared_u32(s_scale_addr + row * Int32(scales_per_row) + (blk & Int32(0x7FFFFFFC)))
        sc_byte = (sc_word >> ((blk & Int32(3)) * Int32(8))) & Uint32(0xFF)
        sf = fp8_e4m3_to_f32(sc_byte)
        h01, h23, h45, h67 = fp4_decode_4bytes(packed)
        f0a, f0b = f16x2_to_f32x2(h01); u0 = pack_f32x2_to_bfloat2(f0a * sf, f0b * sf)
        f1a, f1b = f16x2_to_f32x2(h23); u1 = pack_f32x2_to_bfloat2(f1a * sf, f1b * sf)
        f2a, f2b = f16x2_to_f32x2(h45); u2 = pack_f32x2_to_bfloat2(f2a * sf, f2b * sf)
        f3a, f3b = f16x2_to_f32x2(h67); u3 = pack_f32x2_to_bfloat2(f3a * sf, f3b * sf)
        # 8 channels (8*qj .. 8*qj+7) form one 128b granule: plane = qj//8, vec = qj%8.
        plane_base = cutlass.select_(qj < Int32(granules_per_plane), plane0_base, plane1_base)
        vec = qj % Int32(granules_per_plane)
        granule_off = row * Int32(upcast_stride_plane) + (vec ^ (row % Int32(8)))
        st_shared_v4_u32(plane_base + granule_off * Int32(16), u0, u1, u2, u3)
        i += Int32(num_threads)
...
class PagedForwardKernel:
    ...
    def __init__(
        ...
        *,
        ...
        kv_is_nvfp4: bool = False,
    ):
        self.dtype_q = dtype_q
        self.kv_is_nvfp4 = bool(kv_is_nvfp4)
        self.dtype_kv = dtype_kv
        self.dtype_kv_storage = dtype_kv_storage
        # nvfp4: the cache tensor is uint8 (packed e2m1 + e4m3 scale footer),
        # while the SMEM tile / MMA operand (dtype_kv_storage) is bf16. These
        # differ only for nvfp4; everywhere else they are identical.
        self.kv_cache_elem_dtype = cutlass.Uint8 if bool(kv_is_nvfp4) else dtype_kv_storage
        ...
        # nvfp4: extra SMEM for the packed e2m1 gather buffers + e4m3 scale
        # buffers (the cp.async.bulk + scalar-scale targets); the bf16 plane
        # payload above stays as-is and is filled by the in-SMEM expand.
        # nvfp4 72-wide native (vLLM-native packed cache): the e2m1 (head/2 B) is
        # gathered with ld.global.nc.v2 (8 B, 8-aligned) directly from head*72 --
        # the head stride (head/2 + head/16 = 72) is 8B-aligned for every head, so
        # NO round-down and NO per-head offset are needed (the cp.async.bulk path
        # required a 16B-aligned src and a 16B-rounded 80 B/row superset, which had
        # an elusive odd-head integration bug). The SMEM e2m1 buffer is the NATIVE
        # head/2 = 64 B/row, packed contiguous; the expand reads row*64 + qj*4.
        self.nvfp4_e2m1_row_bytes_k = traits.head_dim_qk // 2
        self.nvfp4_e2m1_row_bytes_v = traits.head_dim_vo // 2
        self.nvfp4_e2m1_stage_bytes_k = self.stage_tile_rows * self.nvfp4_e2m1_row_bytes_k
        self.nvfp4_scale_stage_bytes_k = self.stage_tile_rows * (traits.head_dim_qk // 16)
        self.nvfp4_e2m1_stage_bytes_v = self.stage_tile_rows * self.nvfp4_e2m1_row_bytes_v
        self.nvfp4_scale_stage_bytes_v = self.stage_tile_rows * (traits.head_dim_vo // 16)
        if self.kv_is_nvfp4:
            self.shared_storage_bytes = self.shared_storage_bytes + self.num_stages * (
                self.nvfp4_e2m1_stage_bytes_k
                + self.nvfp4_scale_stage_bytes_k
                + self.nvfp4_e2m1_stage_bytes_v
                + self.nvfp4_scale_stage_bytes_v
            )
        ...
        self.v_tma_plane_count = (
            (traits.head_dim_vo + self.kv_tma_plane_head_dim - 1) // self.kv_tma_plane_head_dim
            if self.use_paged_kv_tma_exact_plane_bf16_layout
            else 1
        )
        ...
        if self.kv_is_nvfp4:
            # ld.global e2m1 + scalar scale are BOTH scalar-gathered into SMEM and
            # made visible by the consumer's post-wait sync_threads (like the
            # scale already was) -- NOTHING rides the mbarrier tx. tx_count=0 so the
            # producer_acquire's plain arrive completes the full mbarrier.
            self.kv_tma_copy_bytes_k = 0
            self.kv_tma_copy_bytes_v = 0
    ...
    @cute.jit
    def _issue_paged_kv_nvfp4_gather(
        self,
        pipeline_tma,
        producer_state,
        mCacheBytes: cute.Tensor,
        ...
        kv_head_idx,
        e2m1_base_u32: Int32,
        scale_base_u32: Int32,
        e2m1_stage_bytes: cutlass.Constexpr,
        scale_stage_bytes: cutlass.Constexpr,
        head_dim: cutlass.Constexpr,
        page_stride_bytes,
        token_stride_bytes,
        head_stride_bytes,
        ...
        """nvfp4 packed-cache producer: replaces the TMA plane copy. Scalar-gathers
        each token's NATIVE 72 B record from the vLLM-native cache with
        ld.global.nc.v2 (8 B, 8-aligned -> works for every head, no round-down, no
        per-head offset): the e2m1 (head_dim/2 B) into the e2m1 SMEM buffer and the
        per-16 e4m3 scale (head_dim/16 B) into the scale buffer. NEITHER rides the
        mbarrier tx (tx_count=0); both are made visible by the consumer's post-wait
        sync_threads. producer_acquire's plain arrive completes the full mbarrier.
        32 lanes x (stage_tile_rows/32) passes.
        """
        page_id = mPageTable[request_idx, page_idx]
        pipeline_tma.producer_acquire(producer_state)
        stage = producer_state.index
        bytes_per_row = head_dim // 2          # e2m1 data (64 for head_dim 128)
        scales_per_row = head_dim // 16        # e4m3 scale (8)
        e2m1_row_bytes = e2m1_stage_bytes // self.stage_tile_rows   # native head/2 (64)
        e2m1_stage_off = Int32(stage) * Int32(e2m1_stage_bytes)
        scale_stage_off = Int32(stage) * Int32(scale_stage_bytes)
        for p in cutlass.range_constexpr(self.stage_tile_rows // 32):
            row = lane + Int32(p * 32)
            entry_idx = sub_tile * Int32(64) + row
            row_byte_base = (
                Int64(page_id) * Int64(page_stride_bytes)
                + Int64(entry_idx) * Int64(token_stride_bytes)
                + Int64(kv_head_idx) * Int64(head_stride_bytes)
            )
            # e2m1: head_dim/2 B = (head_dim/2//16) 16B granules; ld 2x v2 (8B) ->
            # st 1x v4 (16B). src 8-aligned (head*72), dst 16-aligned (row*64).
            e2m1_dst = e2m1_base_u32 + e2m1_stage_off + row * Int32(e2m1_row_bytes)
            for g in cutlass.range_constexpr(bytes_per_row // 16):
                a0, a1 = ld_global_nc_v2_u32(
                    get_ptr_as_int64(mCacheBytes, row_byte_base + Int64(g * 16))
                )
                a2, a3 = ld_global_nc_v2_u32(
                    get_ptr_as_int64(mCacheBytes, row_byte_base + Int64(g * 16 + 8))
                )
                st_shared_v4_u32(e2m1_dst + Int32(g * 16), a0, a1, a2, a3)
            # per-16 e4m3 scale footer (head_dim/16 B), at X + head_dim/2.
            s0, s1 = ld_global_nc_v2_u32(
                get_ptr_as_int64(mCacheBytes, row_byte_base + Int64(bytes_per_row))
            )
            sc_addr = scale_base_u32 + scale_stage_off + row * Int32(scales_per_row)
            st_shared_u32(sc_addr, s0)
            st_shared_u32(sc_addr + Int32(4), s1)
    ...
    def __call__(
        ...
        if const_expr(mKCache.element_type != self.kv_cache_elem_dtype or mVCache.element_type != self.kv_cache_elem_dtype):
            raise TypeError("mKCache/mVCache dtype must match kv_cache_elem_dtype")
        ...
        if const_expr(not self.kv_is_nvfp4):
        ...
        # else nvfp4: no TMA tile descriptor (packed uint8 cache is loaded by the
        # cp.async.bulk gather in the producer). tma_atom_K/V stay None;
        # tma_tensor_K/V stay = mKCacheT/mVCacheT (kernel-signature passthrough).
    ...
    def kernel(
        ...
        kv_chunk_size = mKvChunkSizePtr[0]
        # Block-sparse: cta_tile_q == group_size, so each qo-tile is
        # exactly ONE query. The q2k row used to pick KV blocks is this
        # tile's query: q_start for decode (packed_tile_start==0), and
        # q_start + qo_tile_idx for verify (packed_tile_start =
        # qo_tile_idx*group_size). Using q_start for every verify tile is
        # what made all verify queries attend to query-0 blocks.
        msa_q_row_idx = q_start + packed_tile_start // group_size
        ...
        # --- nvfp4 packed-cache SMEM gather buffers (placed after the bf16 plane
        #     payload; sized into shared_storage_bytes in the ctor). e2m1 buffers
        #     are the cp.async.bulk targets, scale buffers the scalar-gather
        #     targets; both are consumed by the in-SMEM expand into the bf16 planes.
        nvfp4_ke2m1_base = Int32(0)
        nvfp4_kscale_base = Int32(0)
        nvfp4_ve2m1_base = Int32(0)
        nvfp4_vscale_base = Int32(0)
        if const_expr(self.kv_is_nvfp4):
            _nv_pl_base = shared_ptr_to_u32(payload_u8.iterator)
            _nv_off = v_payload_offset + v_payload_bytes
            nvfp4_ke2m1_base = _nv_pl_base + Int32(_nv_off)
            nvfp4_kscale_base = nvfp4_ke2m1_base + Int32(self.num_stages * self.nvfp4_e2m1_stage_bytes_k)
            nvfp4_ve2m1_base = nvfp4_kscale_base + Int32(self.num_stages * self.nvfp4_scale_stage_bytes_k)
            nvfp4_vscale_base = nvfp4_ve2m1_base + Int32(self.num_stages * self.nvfp4_e2m1_stage_bytes_v)
        sKTma = None
        sVTma = None
        ...
        load_K_tma1, _, _ = (
            ...
            if const_expr(self.k_tma_plane_count > 1 and not self.kv_is_nvfp4)
            else (None, None, None)
        )
        load_K_tma2, _, _ = (
            cute_copy.tma_get_copy_fn(
        ...
        load_K_tma0 = (
            cute_copy.tma_producer_copy_fn(load_K_tma0, pipeline_k)
            if const_expr(not self.kv_is_nvfp4)
            else None
        )
        load_K_tma1 = (
        ...
        load_V_tma1, _, _ = (
            ...
            if const_expr(self.v_tma_plane_count > 1 and not self.kv_is_nvfp4)
            else (None, None, None)
        )
        load_V_tma2, _, _ = (
            cute_copy.tma_get_copy_fn(
        ...
        load_V_tma0 = (
            cute_copy.tma_producer_copy_fn(load_V_tma0, pipeline_v)
            if const_expr(not self.kv_is_nvfp4)
            else None
        )
        load_V_tma1 = (
        ...
        for _prefetch_stage in cutlass.range_constexpr(initial_prefetch_stages):
            if prefetch_base < chunk_end:
                ...
                if warp_linear_idx == Int32(0):
                    ...
                    if const_expr(self.kv_is_nvfp4):
                        self._issue_paged_kv_nvfp4_gather(
                            pipeline_k, k_producer_state, mKCache, mPageTable, mbar_ptr_K,
                            request_idx, prefetch_page_idx, prefetch_sub_tile, kv_head_idx,
                            nvfp4_ke2m1_base, nvfp4_kscale_base,
                            self.nvfp4_e2m1_stage_bytes_k, self.nvfp4_scale_stage_bytes_k,
                            self.traits.head_dim_qk,
                            mKCache.stride[0], mKCache.stride[1], mKCache.stride[2], lane,
                        )
                    elif const_expr(self.k_tma_plane_count > 3):
                    ...
                    if const_expr(self.kv_is_nvfp4):
                        self._issue_paged_kv_nvfp4_gather(
                            pipeline_v, v_producer_state, mVCache, mPageTable, mbar_ptr_V,
                            request_idx, prefetch_page_idx, prefetch_sub_tile, kv_head_idx,
                            nvfp4_ve2m1_base, nvfp4_vscale_base,
                            self.nvfp4_e2m1_stage_bytes_v, self.nvfp4_scale_stage_bytes_v,
                            self.traits.head_dim_vo,
                            mVCache.stride[0], mVCache.stride[1], mVCache.stride[2], lane,
                        )
                    elif const_expr(self.v_tma_plane_count > 3):
                        self._issue_paged_kv_tma_copy_planes(
                            load_V_tma0,
                            load_V_tma1,
                            load_V_tma2,
        ...
        while tile_base < chunk_end:
            ...
            if const_expr(self.bf16_minimax_role_specialized_decode):
                ...
                cute.arch.sync_threads()
            if const_expr(self.kv_is_nvfp4):
                # consumer_wait acquired the e2m1 bulk; the sync above made the
                # scalar-gathered scale visible. Expand e2m1xscale -> the bf16 K
                # planes (where the stock bf16 QK MMA reads), then re-sync.
                _expand_nvfp4_to_bf16_planes(
                    nvfp4_ke2m1_base + Int32(consume_stage_idx * self.nvfp4_e2m1_stage_bytes_k),
                    nvfp4_kscale_base + Int32(consume_stage_idx * self.nvfp4_scale_stage_bytes_k),
                    shared_ptr_to_u32(
                        sKStageBytes.iterator + Int32(consume_stage_idx * kv_plane_stage_bytes) + Int32(0 * kv_plane_total_bytes)
                    ),
                    shared_ptr_to_u32(
                        sKStageBytes.iterator + Int32(consume_stage_idx * kv_plane_stage_bytes) + Int32(1 * kv_plane_total_bytes)
                    ),
            ...
            for _ in cutlass.range_constexpr(1):
                        ...
                        if next_tile_base < chunk_end:
                            if warp_linear_idx == Int32(0):
                                ...
                                if const_expr(self.kv_is_nvfp4):
                                    self._issue_paged_kv_nvfp4_gather(
                                        pipeline_k, k_producer_state, mKCache, mPageTable, mbar_ptr_K,
                                        request_idx, next_page_idx, next_sub_tile, kv_head_idx,
                                        nvfp4_ke2m1_base, nvfp4_kscale_base,
                                        self.nvfp4_e2m1_stage_bytes_k, self.nvfp4_scale_stage_bytes_k,
                                        self.traits.head_dim_qk,
                                        mKCache.stride[0], mKCache.stride[1], mKCache.stride[2], lane,
                ...
                if const_expr(self.kv_is_nvfp4):
                    # V stores in the SAME _permuted_offset_128b layout as K (the
                    # PV MMA only differs by a transposed ldmatrix read), so the
                    # same expand fills the bf16 V planes. Sync first (make the
                    # scalar V scale visible), then re-sync before the PV MMA.
                    cute.arch.sync_threads()
                    _expand_nvfp4_to_bf16_planes(
                        nvfp4_ve2m1_base + Int32(consume_stage_idx * self.nvfp4_e2m1_stage_bytes_v),
                        nvfp4_vscale_base + Int32(consume_stage_idx * self.nvfp4_scale_stage_bytes_v),
                        shared_ptr_to_u32(
                            sVStageBytes.iterator + Int32(consume_stage_idx * kv_plane_stage_bytes) + Int32(0 * kv_plane_total_bytes)
                        ),
                        shared_ptr_to_u32(
                            sVStageBytes.iterator + Int32(consume_stage_idx * kv_plane_stage_bytes) + Int32(1 * kv_plane_total_bytes)
                        ),
                        tc_upcast_stride_plane,
                        tidx,
                        self.traits.num_threads,
