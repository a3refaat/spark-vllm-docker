...
def _build_forward_kernel(
    ...
    # nvfp4: the SMEM K/V tile (sKPlane) stays bf16 -- it is the EXPAND TARGET
    # the stock bf16 MMA reads. The uint8 packed cache is recognized via
    # kv_is_nvfp4 + kv_cache_elem_dtype in the kernel and loaded by a
    # cp.async.bulk e2m1 gather + scalar e4m3-scale gather + in-SMEM
    # e2m1xscale->bf16 expand (bf16 traits => dtype_kv_storage == bf16 here).
    dtype_kv_storage = _torch_to_cutlass_storage_dtype(traits.kv_dtype)
...
def paged_attention_forward(
    ...
    *,
    ...
    kv_is_nvfp4 = getattr(plan, "kv_quant", "none") == "nvfp4"
    if page_size == 128:
        if kv_is_nvfp4:
            kv_ok = k_cache.dtype == torch.uint8 and v_cache.dtype == torch.uint8
        else:
            kv_ok = (
                k_cache.dtype in (torch.float16, torch.bfloat16, torch.float8_e4m3fn)
                and v_cache.dtype == k_cache.dtype
            )
        if not kv_ok:
            raise ValueError(
                "page_size=128 paged attention requires matching fp16, bf16, fp8 e4m3, "
                "or nvfp4 (uint8) K/V caches"
            )
    ...
    if plan.mode == "extend":
        ...
        # The specialized decode-graph split kernels assume one KV tile per
        # page-table entry (page_size=64 for this trait family; validated on
        # mimo_v25 diffkv). page_size=128 (page_tiles_per_entry=2) dense split
        # decode produces wrong partials on these variants -- route it through
        # the generic split forward + merge instead (validated in
        # tests/test_paged_dense_decode_split_kv.py).
        specialized_decode_graph_ok = page_tiles_per_entry == 1
        single_request_decode_graph = (
            plan.mode == "decode"
            and plan.enable_cuda_graph
            and plan.split_kv
            and not bool(plan.msa_block_sparse)
            and specialized_decode_graph_ok
    ...
    k_cache_arg = (
        ...
        if k_cache.dtype in (torch.float8_e4m3fn, torch.uint8)
    ...
    v_cache_arg = (
        ...
        if v_cache.dtype in (torch.float8_e4m3fn, torch.uint8)
    ...
    if plan.split_kv:
        ...
        merge_regular_decode_graph = (
            plan.mode == "decode"
            and plan.enable_cuda_graph
            # pair the regular/direct-grid merge with the specialized forward
            # variants only (page_tiles_per_entry == 1); the generic split
            # forward is merged by the generic indptr-driven merge.
            and page_tiles_per_entry == 1
            and plan.gqa_group_size <= plan.cta_tile_q
            and workspace._decode_graph_chunk_pages_lut is not None
