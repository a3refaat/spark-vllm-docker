...
from b12x.cute.fp4 import (
    ...
    shared_ptr_to_u32,
    st_shared_v4_u32,
    # nvfp4 (bf16) indexer path (S5): expand + bf16 m16n8k16 QK
...
def _paged_indexer_shared_storage_cls(
    ...
    # S5 nvfp4 path: q-stage + k-stage are bf16 (2 bytes/elem); no permuted K
    # stage and no per-token scale buffer (block scale folds into expanded k).
    q_bytes_elems = int(padded_q_heads) * _INDEX_HEAD_DIM * (2 if is_nv else 1)
    annotations = {
        "mbar_ptr_k": cute.struct.MemRange[cutlass.Int64, 1],
        "q_bytes": cute.struct.Align[
            cute.struct.MemRange[cutlass.Uint8, q_bytes_elems],
            16,
        ],
    ...
    if is_nv:
        # bf16 expanded K stage (2 bytes/elem); reuses the k_page slot.
        annotations["k_page"] = cute.struct.Align[
            cute.struct.MemRange[cutlass.Uint8, _PAGE_SIZE * _INDEX_HEAD_DIM * 2],
            1024,
        ]
    else:
        annotations["k_page"] = k_page_storage()
    # k_page_perm + scales kept for both dtypes so the shared top-of-kernel setup
    # needs no branch (unused for nvfp4: the bf16 path skips repack/per-token scale).
    annotations["k_page_perm"] = k_page_storage()
    annotations["scales"] = cute.struct.Align[
...
def _needs_paged_index_k_scalar_load(
    ...
    # S5: nvfp4 caches are always loaded+expanded cooperatively (no TMA path).
    if index_k_cache.ndim == 2 and index_k_cache.shape[1] == _PAGE_SIZE * (
        _INDEX_HEAD_DIM // 2 + _INDEX_HEAD_DIM // 16
    ):
        return True
...
@cute.jit
def _load_and_expand_index_k_page_nvfp4(
    k_e2m1_bytes: cute.Tensor,    # [pages, 64, 64] uint8 e2m1 (2 ch/byte, even=low)
    k_scale_bytes: cute.Tensor,   # [pages, 64, 8] uint8 e4m3 per-16-block scale
    page_id: Int32,
    s_k_bf16_base: Int32,         # bf16 [64,128] row-major k-stage (SMEM addr)
    lane_linear: Int32,
):
    """S5: load one nvfp4 index-K page (e2m1 + per-16 e4m3) and expand to a bf16
    [64,128] row-major SMEM stage. Each e2m1 byte -> 2 bf16 channels; the per-16
    block scale folds into the value (no per-token scale downstream). Proven
    bit-exact in tools/nvfp4-kv/bf16_nvfp4_qk_micro.py."""
    linear = lane_linear
    total = Int32(_PAGE_SIZE * (_INDEX_HEAD_DIM // 2))   # 64 tok * 64 e2m1 bytes
    while linear < total:
        tok = linear // Int32(_INDEX_HEAD_DIM // 2)
        j = linear - tok * Int32(_INDEX_HEAD_DIM // 2)   # 0..63 -> channels 2j,2j+1
        ...
        st_shared_u32(s_k_bf16_base + (tok * Int32(_INDEX_HEAD_DIM) + j * Int32(2)) * Int32(2), bf)
        linear += Int32(_PAGED_THREADS_PER_CTA)


@cute.jit
def _compute_bf16_tile_partials(
    s_q_bf16_base: Int32,        # bf16 [padded_q_heads,128] row-major q-stage (SMEM addr)
    s_w: cute.Tensor,
    num_heads: Int32,
    ...
    """S5 bf16 port of _compute_mxfp8_tile_partials. Same n8 token-grouping +
    head-sum epilogue (identical D n-layout); only the inner contraction is
    bf16 m16n8k16 (q via ldmatrix from the bf16 q-stage; k = manual bf16 pairs
    from the bf16 k-stage; 8 k16 iters). q,k are full precision so the per-token
    q_scale/k_scale are gone (block scale folded into k); the head weight w (=
    MSA_SM_SCALE for the MSA path) is applied in the epilogue, unchanged.
    Frag layout proven in tools/nvfp4-kv/bf16_qk_micro.py."""
    group_id = lane // Int32(4)
    thread_id_in_group = lane % Int32(4)
    col_pair_base = thread_id_in_group * Int32(2)
    a_row = (lane & Int32(7)) + ((lane >> Int32(3)) & Int32(1)) * Int32(8)
    a_col = (lane >> Int32(4)) * Int32(8)
    b_token = token_base + group_id                  # B n-index = lane//4
    q0_acc = Float32(0.0)
    q1_acc = Float32(0.0)
    q2_acc = Float32(0.0)
    q3_acc = Float32(0.0)
    for kk in cutlass.range_constexpr(_INDEX_HEAD_DIM // 16):
        ...
        a0, a1, a2, a3 = ldmatrix_m8n8x4_b16(
            s_q_bf16_base + ((head_tile_base + a_row) * Int32(_INDEX_HEAD_DIM) + a_col + k0) * Int32(2)
        )
        b0 = ld_shared_u32(k_bf16_base + (b_token * Int32(_INDEX_HEAD_DIM) + k0 + col_pair_base) * Int32(2))
        b1 = ld_shared_u32(k_bf16_base + (b_token * Int32(_INDEX_HEAD_DIM) + k0 + Int32(8) + col_pair_base) * Int32(2))
...
class SparseNSAPagedLogitsKernel:
    ...
    def __init__(
        ...
        *,
        ...
        self.kv_quant = str(kv_quant)   # S5: "nvfp4" -> bf16 expand + bf16 QK
    ...
    def kernel(
        ...
        # S5: hoist the q-stage SMEM base addr OUT of the dynamic `if cta_idx`
        # block (all storage.* member access must be hoisted, like the fp8 path).
        q_bytes_base_addr = shared_ptr_to_u32(storage.q_bytes.data_ptr())
        if cta_idx < total_work:
            ...
            if cutlass.const_expr(self.kv_quant == "nvfp4"):
                # Expand fp8 q -> bf16 q-stage (the bf16 MMA needs bf16 q). q stays
                # fp8 on the wire; q_scale rides the weights exactly like the fp8
                # path, so the API/caller are unchanged.
                total_pairs = Int32(self.padded_q_heads * (_INDEX_HEAD_DIM // 2))
                while q_linear < total_pairs:
                    head_idx = q_linear // Int32(_INDEX_HEAD_DIM // 2)
                    pair = q_linear - head_idx * Int32(_INDEX_HEAD_DIM // 2)
                    c0 = pair * Int32(2)
                    bf = Uint32(0)
                    if head_idx < num_heads:
                        f0 = fp8_e4m3_to_f32(Uint32(q_bytes[q_idx, head_idx, c0]))
                        f1 = fp8_e4m3_to_f32(Uint32(q_bytes[q_idx, head_idx, c0 + Int32(1)]))
                        bf = pack_f32x2_to_bfloat2(f0, f1)
                    st_shared_u32(
                        q_bytes_base_addr + (head_idx * Int32(_INDEX_HEAD_DIM) + c0) * Int32(2), bf
                    )
            ...
            while work_idx < total_work:
                ...
                if (page_base < seq_len) & (source_page_col < Int32(real_page_table.shape[1])):
                    ...
                    if page_id >= Int32(0):
                        if cutlass.const_expr(self.kv_quant == "nvfp4"):
                            # S5: load e2m1 page + expand x e4m3 -> bf16 k-stage (no
                            # TMA/mbar, no repack, no per-token scale).
                            _load_and_expand_index_k_page_nvfp4(
                                k_quant_bytes, k_scales, page_id, k_page_base_addr, tx
                            )
                            cute.arch.sync_threads()
                        else:
                        ...
                        while split_idx < Int32(self.page_splits):
                            ...
                            if (head_tile_slot == Int32(0)) & (lane < Int32(_PAGED_TOKENS_PER_GROUP)):
                                ...
                                if slot_idx < valid_slots:
                                    ...
                                    if cutlass.const_expr(self.kv_quant == "nvfp4"):
                                        value = logit   # block scale folded into bf16 k; no per-token scale
                                    else:
...
class SparseNSAScheduledSingleRowLogitsKernel:
    ...
    def __init__(
        ...
        if self.kv_quant == "nvfp4" and self.output_mode == IndexerOutputMode.PAGE_HEAD_MAX:
            raise ValueError("nvfp4 scheduled single-row kernel only supports TOKEN_LOGITS output")
        self.padded_q_heads = self.num_q_head_tiles * _PAGED_Q_HEAD_TILE
        self.token_groups = _PAGED_WARPS_PER_CTA // self.num_q_head_tiles
        self.tokens_per_work = _PAGED_TOKENS_PER_GROUP * self.token_groups
        self.page_splits = _PAGE_SIZE // self.tokens_per_work
    ...
    def kernel(
        ...
        # S5: hoist q-stage SMEM base addr OUT of the dynamic schedule `if`
        # (all storage.* access must be hoisted, like k_page_base_addr above).
        q_bytes_base_addr = shared_ptr_to_u32(storage.q_bytes.data_ptr())
        ...
        if (
            ...
            q_linear = tx
            if cutlass.const_expr(self.kv_quant == "nvfp4"):
                # Expand fp8 q -> bf16 q-stage (bf16 MMA needs bf16 q). q stays
                # fp8 on the wire; q_scale rides the weights exactly like fp8.
                total_pairs = Int32(self.padded_q_heads * (_INDEX_HEAD_DIM // 2))
                while q_linear < total_pairs:
                    head_idx = q_linear // Int32(_INDEX_HEAD_DIM // 2)
                    pair = q_linear - head_idx * Int32(_INDEX_HEAD_DIM // 2)
                    ...
                    bf = Uint32(0)
                    if head_idx < num_heads:
                        f0 = fp8_e4m3_to_f32(Uint32(q_bytes[Int32(0), head_idx, c0]))
                        f1 = fp8_e4m3_to_f32(Uint32(q_bytes[Int32(0), head_idx, c0 + Int32(1)]))
                        bf = pack_f32x2_to_bfloat2(f0, f1)
                    st_shared_u32(
                        q_bytes_base_addr + (head_idx * Int32(_INDEX_HEAD_DIM) + c0) * Int32(2), bf
                    )
            ...
            while page_col < interval_page_end:
                ...
                if page_base < seq_len:
                    ...
                    if page_id >= Int32(0):
                        if cutlass.const_expr(self.kv_quant == "nvfp4"):
                            # S5: load e2m1 page + expand x e4m3 -> bf16 k-stage
                            # (no TMA/mbar, no repack, no per-token scale).
                            _load_and_expand_index_k_page_nvfp4(
                                k_quant_bytes, k_scales, page_id, k_page_base_addr, tx
                            )
                            cute.arch.sync_threads()
                        ...
                        while split_idx < Int32(self.page_splits):
                            ...
                            if token_base < valid_slots:
                                ...
                                elif cutlass.const_expr(self.kv_quant == "nvfp4"):
                                    _compute_bf16_tile_partials(
                                        q_bytes_base_addr,
                                        s_w,
                                        num_heads,
                                ...
                                if (head_tile_slot == Int32(0)) & (lane < Int32(_PAGED_TOKENS_PER_GROUP)):
                                    ...
                                    if slot_idx < valid_slots:
                                        ...
                                        while head_tile_idx < Int32(self.num_q_head_tiles):
                                            ...
                                            head_tile_idx += Int32(1)
                                        if cutlass.const_expr(self.kv_quant == "nvfp4"):
                                            # block scale folded into bf16 k; no per-token scale
                                            logits_out[Int32(0), page_base + slot_idx] = logit
                                        else:
...
def _build_sparse_nsa_paged_kernel(
    ...
    return SparseNSAPagedLogitsKernel(
        persistent_ctas, num_heads_static, score_mode=score_mode, kv_quant=kv_quant
    )
...
def _split_index_k_cache_runtime_views(index_k_cache: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    # S5: nvfp4 L1 page-major cache row = [64*64 e2m1 | 64*8 e4m3] (4608); detect
    # by width and return e2m1 [pages,64,64] + e4m3 scale [pages,64,8] (uint8).
    if index_k_cache.shape[1] == _PAGE_SIZE * (_INDEX_HEAD_DIM // 2 + _INDEX_HEAD_DIM // 16):
        e2m1_bytes = _PAGE_SIZE * (_INDEX_HEAD_DIM // 2)
        k_quant_bytes = index_k_cache[:, :e2m1_bytes].view(
            num_pages, _PAGE_SIZE, _INDEX_HEAD_DIM // 2
        )
        k_scales = index_k_cache[:, e2m1_bytes:].view(
            num_pages, _PAGE_SIZE, _INDEX_HEAD_DIM // 16
        )
...
def supports_paged_logits_kernel(
    *,
    ...
    if seqlens_per_query.ndim != 1 or seqlens_per_query.shape[0] != q_fp8.shape[0]:
        return False
    if index_k_cache.ndim != 2 or index_k_cache.shape[1] not in (
        _PAGE_SIZE * (_INDEX_HEAD_DIM + _SCALE_BYTES),                 # fp8 row
        _PAGE_SIZE * (_INDEX_HEAD_DIM // 2 + _INDEX_HEAD_DIM // 16),   # S5 nvfp4 row
    ):
        return False
...
def run_paged_logits_kernel(
    *,
    ...
    # S5: nvfp4 index-K is implemented only on the base (non-scheduled) kernel;
    # force it past the scheduled single/multi-row routes (which 2D-index the
    # per-token fp8 scale and would mis-read the 3D nvfp4 block scales).
    _paged_nvfp4 = index_k_cache.shape[1] == _PAGE_SIZE * (
        _INDEX_HEAD_DIM // 2 + _INDEX_HEAD_DIM // 16
    )
    if _should_use_schedule_single_row_kernel(q_rows=rows, max_pages=max_pages):
        ...
        # S5: nvfp4 now supported on the single-row scheduled kernel (bf16 expand).
        kernel = _build_sparse_nsa_schedule_single_row_kernel(
            _SCHEDULE_SINGLE_ROW_PARALLEL_CTAS,
            q_fp8.shape[1],
            score_mode,
            output_mode,
            "nvfp4" if _paged_nvfp4 else "none",
        )
        args = (
            *common_args,
    ...
    elif (not _paged_nvfp4) and _should_use_schedule_multi_row_kernel(q_rows=rows, max_pages=max_pages):
        if schedule_metadata is None:
        ...
        kernel = _build_sparse_nsa_paged_kernel(
            ...
            score_mode,
            "nvfp4"
            if index_k_cache.shape[1]
            == _PAGE_SIZE * (_INDEX_HEAD_DIM // 2 + _INDEX_HEAD_DIM // 16)
            else "none",
        )
        args = (
            *common_args,
