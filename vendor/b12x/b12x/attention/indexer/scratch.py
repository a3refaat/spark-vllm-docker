...
class B12XIndexerPagedScratchCaps:
    ...
    score_mode: str = "nsa"
    num_idx_heads: int = 1
    kv_quant: str = "none"   # S5: "nvfp4" -> bf16-expand indexer K (single source of truth)
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "score_mode", score_mode)
        kv_quant = str(self.kv_quant).lower()
        if kv_quant not in ("none", "nvfp4"):
            raise ValueError(f"indexer paged scratch kv_quant must be none or nvfp4, got {kv_quant!r}")
        object.__setattr__(self, "kv_quant", kv_quant)
    ...
    if route == INDEXER_PAGED_ROUTE_AUTO:
        if bool(caps.shared_page_table) or str(caps.mode) == "prefill":
            ...
            if os.getenv("B12X_FUSED_INDEXER", "1") != "0":
                from b12x.attention.indexer.fused_indexer import resolve_fused_indexer_path
                from b12x.attention.indexer.kernel import _num_q_head_tiles
...
class B12XIndexerPagedScratchPlan:
    ...
    def resolved_policy(self) -> PagedDecodePolicy | None:
        """Descriptive decode policy resolved at plan time (never behavioral)."""
    ...
    def bind_msa(
        ...
        *,
        ...
        return build_indexer_msa_paged_binding(
            ...
            schedule_metadata=schedule_metadata,
            topk=topk,
            kv_quant=getattr(self.caps, "kv_quant", "none"),
        )
...
def _resolve_indexer_paged_policy(
    caps: B12XIndexerPagedScratchCaps,
) -> PagedDecodePolicy:
    """Describe the backend the paged indexer will select for these caps.

    Mirrors the kernel's own scheduled-path gating (single-row / multi-row
    schedule kernels need a page-table width of at least _SCHEDULE_MIN_PAGES
    page-64 rows; the nvfp4 index-K cache always takes the bf16-expand base
    path today). Purely descriptive -- selection stays in the kernel.
    """
    backend_variant = "base"
    if caps.score_mode == "msa" and caps.kv_quant == "none":
        from b12x.attention.indexer.kernel import (
            _should_use_schedule_multi_row_kernel,
            _should_use_schedule_single_row_kernel,
        )

        if _should_use_schedule_single_row_kernel(
            q_rows=caps.max_q_rows, max_pages=caps.max_page_table_width
        ) or _should_use_schedule_multi_row_kernel(
            q_rows=caps.max_q_rows, max_pages=caps.max_page_table_width
        ):
            backend_variant = "scheduled"
    return PagedDecodePolicy(
        family="indexer",
        mode=caps.mode,
        # score_mode="msa" stores packed fp8 index-K rows unless nvfp4 packing
        # is selected; kv_dtype is the raw byte storage either way.
        kv_quant=(
            "nvfp4"
            if caps.kv_quant == "nvfp4"
            else ("fp8" if caps.score_mode == "msa" else "none")
        ),
        kv_dtype=str(caps.kv_dtype),
        page_size=int(caps.page_size),
        head_dim_qk=0,
        head_dim_vo=0,
        gqa_group_size=int(caps.num_q_heads) // max(int(caps.num_idx_heads), 1),
        max_batch=int(caps.max_batch),
        max_total_q=int(caps.max_q_rows),
        max_page_table_width=int(caps.max_page_table_width),
        cache_page_capacity=int(caps.max_page_table_width),
        split_kv=False,
        chunk_pages=None,
        max_chunks_per_request=None,
        graph_ctas_per_sm=None,
        metadata_strategy="runtime-update",
...
def plan_indexer_paged_scratch(
    ...
    policy = _resolve_indexer_paged_policy(caps)
    emit_resolved_policy(policy, site="indexer_scratch.paged")
