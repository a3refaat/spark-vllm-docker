...
def paged_decode_logits(
    *,
    ...
    # S5: nvfp4 index-K is supported on the base AND the single-row scheduled
    # kernel (bf16 expand), but NOT the multi-row scheduled kernel -- so only
    # skip the schedule (force base) for nvfp4 multi-row decode (q_rows > 1).
    _paged_nvfp4 = index_k_cache.shape[1] == page_size * (
        _INDEX_HEAD_DIM // 2 + _INDEX_HEAD_DIM // 16
    )
    _skip_schedule = _paged_nvfp4 and valid_q_rows > 1
    if (not _skip_schedule) and uses_paged_mqa_schedule(
        q_rows=valid_q_rows,
...
def msa_paged_decode_block_scores(
    *,
    ...
    # S5: nvfp4 indexer K runs the bf16-expand scorer, which lives on the
    # non-scheduled (TOKEN_LOGITS) route -> force the fallback. kv_quant comes
    # from the explicit kwarg (metadata path) or binding.kv_quant (mod path).
    _binding_kv_quant = getattr(binding, "kv_quant", "none") if binding is not None else "none"
    _is_nvfp4 = str(kv_quant if kv_quant is not None else _binding_kv_quant) == "nvfp4"
    max_pages = int(metadata.real_page_table.shape[1])
    use_page_max = (
