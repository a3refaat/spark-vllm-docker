...
from b12x.cute.fp4 import (
    ...
    mxf4nvf4_mma_m16n8k64_f32_e2m1,
...
# Native NVFP4 indexer-K scorer (parallel to the e4m3 path; selected only for the
# nvfp4 indexer cache).  e2m1 K is 64 B/token; per-16 ue4m3 scales are 8 B/token.
_NVF4_HEAD_U32 = _INDEX_HEAD_DIM // 8     # 16 u32/token (e2m1, 64 B)
_NVF4_NBLK = _INDEX_HEAD_DIM // 16        # 8 per-16 micro-block scales/token
_NVF4_NBLK_U32 = _NVF4_NBLK // 4          # 2 u32/token of packed ue4m3 scales
_NVF4_BLOCK_K = 128       # one MSA 128-token block per CTA -> block_idx == k_tile_idx
_NVF4_WARPS_K = 4         # each K-warp covers 32 K-tokens (num_mma_kv=2 x 16)
_NVF4_NUM_MMA_KV = _NVF4_BLOCK_K // (_NVF4_WARPS_K * 16)   # 2
_NVF4_WARPS_PER_CTA = _WARPS_Q * _NVF4_WARPS_K            # 8
_NVF4_THREADS_PER_CTA = _NVF4_WARPS_PER_CTA * _WARP_THREADS  # 256
...
# =============================================================================
# Native NVFP4 indexer-K MSA block-score scorer (parallel to the e4m3 path above).
#
# Selected ONLY for the nvfp4 indexer cache.  Q is e2m1 + per-16 ue4m3 (SFA),
# K is e2m1 + per-16 ue4m3 (SFB); the mxf4nvf4 k64 MMA applies BOTH scales in
# hardware, so there is no per-token q_scale/k_scale.  MSA is PER-HEAD: this
# kernel mirrors the e4m3 prefill block-score path (SparseNSAContiguousLogits-
# PrefillKernel, BLOCK_SCORE_OUTPUT=True) -- per head, block-max over a 128-token
# K block, write block_scores[head, q_row, block].  BK=128 => block == k_tile_idx.
# The native QK produces the identical 8-reg/16-k score_frag layout as
# _prefill_qk_mma_from_smem_q, so the block-max epilogue is reused unchanged.
# The caller folds only MSA_SM_SCALE into `weights` (q-scale is hardware).
# Validated datapath: tools/nvfp4-kv/nvf4_qk_smem_probe.py.
# =============================================================================


@lru_cache(maxsize=1)
def get_sparse_nsa_nvf4_contiguous_shared_storage_cls():
    class SharedStorage:
        pass

    SharedStorage.__annotations__ = {
        "tile_live": cute.struct.Align[cute.struct.MemRange[cutlass.Int32, 1], 16],
        "k_e2m1": cute.struct.Align[
            cute.struct.MemRange[cutlass.Uint32, _NVF4_BLOCK_K * _NVF4_HEAD_U32], 1024
        ],
        "k_sfb": cute.struct.Align[
            cute.struct.MemRange[cutlass.Uint32, _NVF4_BLOCK_K * _NVF4_NBLK_U32], 16
        ],
        "k_start": cute.struct.Align[cute.struct.MemRange[cutlass.Int32, _BLOCK_Q], 16],
        "k_end": cute.struct.Align[cute.struct.MemRange[cutlass.Int32, _BLOCK_Q], 16],
        "block_partial": cute.struct.Align[
            cute.struct.MemRange[cutlass.Float32, _NVF4_WARPS_K * _BLOCK_Q], 16
        ],
    }
    return cute.struct(SharedStorage)
...
@cute.jit
def _qk_mma_nvf4(
    score_frag: cute.Tensor,     # [1, _NVF4_NUM_MMA_KV, 8]  per-head q.k (raw, hw-scaled)
    q_e2m1: cute.Tensor,         # global [Qp, H, 16] u32  (e2m1, 64 B/token,head)
    q_sfa: cute.Tensor,          # global [Qp, H, 2]  u32  (per-16 ue4m3, 8 B)
    head_idx: Int32,
    q_tile_base: Int32,
    valid_q_rows: Int32,
    sK: cute.Tensor,             # smem  [_NVF4_BLOCK_K * 16] u32 e2m1 K tile (token-major)
    sKsf: cute.Tensor,           # smem  [_NVF4_BLOCK_K * 2]  u32 per-16 ue4m3 K scales
    lane,
    warp_q_idx,
    warp_k_idx,
) -> None:
    """One warp's 16 x (num_mma_kv*16) q.k tile via mxf4nvf4 k64 MMAs.

    Emits the SAME 8-reg/16-k layout as the e4m3 _prefill_qk_mma_from_smem_q:
    per mma_kv, two n8 MMAs (cols 0-7 -> regs 0-3, cols 8-15 -> regs 4-7), each
    the K=128 sum of 2 mxf4nvf4 k64 pairs.  D fragment: d0=(g,2c) d1=(g,2c+1)
    d2=(g+8,2c) d3=(g+8,2c+1).
    """
    g = lane // Int32(4)
    c = lane % Int32(4)
    sfa_g = g + Int32(8) * (c & Int32(1))
    for mma_kv in cutlass.range_constexpr(_NVF4_NUM_MMA_KV):
        for n8 in cutlass.range_constexpr(2):
            d0 = Float32(0.0)
            d1 = Float32(0.0)
            d2 = Float32(0.0)
            d3 = Float32(0.0)
            kcol = (
                warp_k_idx * Int32(_NVF4_NUM_MMA_KV * 16)
                + Int32(mma_kv * 16)
                + Int32(n8 * 8)
                + g
            )
            for p in cutlass.range_constexpr(2):
                wlo = Int32(8 * p) + c
                whi = Int32(8 * p + 4) + c
                qr0 = q_tile_base + warp_q_idx * Int32(16) + g
                qr8 = qr0 + Int32(8)
                a0 = Uint32(q_e2m1[qr0, head_idx, wlo]) if qr0 < valid_q_rows else Uint32(0)
                a1 = Uint32(q_e2m1[qr8, head_idx, wlo]) if qr8 < valid_q_rows else Uint32(0)
                a2 = Uint32(q_e2m1[qr0, head_idx, whi]) if qr0 < valid_q_rows else Uint32(0)
                a3 = Uint32(q_e2m1[qr8, head_idx, whi]) if qr8 < valid_q_rows else Uint32(0)
                b0 = Uint32(sK[kcol * Int32(_NVF4_HEAD_U32) + wlo])
                b1 = Uint32(sK[kcol * Int32(_NVF4_HEAD_U32) + whi])
                sfa_qrow = q_tile_base + warp_q_idx * Int32(16) + sfa_g
                sfa = (
                    Uint32(q_sfa[sfa_qrow, head_idx, Int32(p)])
                    if sfa_qrow < valid_q_rows
                    else Uint32(0)
                )
                sfb = Uint32(sKsf[kcol * Int32(_NVF4_NBLK_U32) + Int32(p)])
                d0, d1, d2, d3 = mxf4nvf4_mma_m16n8k64_f32_e2m1(
                    d0, d1, d2, d3, a0, a1, a2, a3, b0, b1, sfa, sfb
                )
            score_frag[0, mma_kv, n8 * 4 + 0] = d0
            score_frag[0, mma_kv, n8 * 4 + 1] = d1
            score_frag[0, mma_kv, n8 * 4 + 2] = d2
            score_frag[0, mma_kv, n8 * 4 + 3] = d3


class SparseNSANvf4ContiguousBlockScoresKernel:
    """Native-NVFP4 MSA per-head block-score scorer (BLOCK_K=128 == one MSA block).

    Q/K are e2m1 + per-16 ue4m3; the k64 MMA applies both scales in hardware.
    Cooperative global->smem K staging (no TMA in this first cut; TMA is a
    measured perf pass).  Parallel to the e4m3 kernels -- fp8 path untouched.
    """

    def __init__(self, *, score_mode: int = IndexerScoreMode.MSA_BILINEAR):
        self._score_mode = int(score_mode)
    ...
    def __call__(
        ...
        self.kernel(
            q_e2m1, q_sfa, weights, k_e2m1, k_sfb, k_start, k_end, block_scores,
            valid_q_rows, valid_k_rows, num_blocks_out,
        ).launch(
            grid=(
                (valid_q_rows + _BLOCK_Q - 1) // _BLOCK_Q,
                (valid_k_rows + _NVF4_BLOCK_K - 1) // _NVF4_BLOCK_K,
                1,
            ),
            block=[_NVF4_THREADS_PER_CTA, 1, 1],
            stream=stream,
        )
    ...
    def kernel(
        ...
        lane = tx % Int32(_WARP_THREADS)
        warp_idx = tx // Int32(_WARP_THREADS)
        warp_k_idx = warp_idx % Int32(_NVF4_WARPS_K)
        warp_q_idx = warp_idx // Int32(_NVF4_WARPS_K)

        q_tile_base = q_tile_idx * Int32(_BLOCK_Q)
        k_tile_base = k_tile_idx * Int32(_NVF4_BLOCK_K)
        valid_q_rows = Int32(valid_q_rows)
        k_total_rows = Int32(valid_k_rows)
        num_heads = Int32(q_e2m1.shape[1])

        smem = cutlass.utils.SmemAllocator()
        storage = smem.allocate(get_sparse_nsa_nvf4_contiguous_shared_storage_cls())
        sK = storage.k_e2m1.get_tensor(
            cute.make_layout((_NVF4_BLOCK_K * _NVF4_HEAD_U32,), stride=(1,))
        )
        sKsf = storage.k_sfb.get_tensor(
            cute.make_layout((_NVF4_BLOCK_K * _NVF4_NBLK_U32,), stride=(1,))
        )
        s_k_start = storage.k_start.get_tensor(cute.make_layout((_BLOCK_Q,), stride=(1,)))
        s_k_end = storage.k_end.get_tensor(cute.make_layout((_BLOCK_Q,), stride=(1,)))
        s_tile_live = storage.tile_live.get_tensor(cute.make_layout((1,), stride=(1,)))
        s_block_partial = storage.block_partial.get_tensor(
            cute.make_layout((_NVF4_WARPS_K, _BLOCK_Q), stride=(_BLOCK_Q, 1))
        )
        ...
        while row_linear < Int32(_BLOCK_Q):
            ...
            row_linear += Int32(_NVF4_THREADS_PER_CTA)
        cute.arch.sync_threads()

        if warp_idx == Int32(0):
            tile_k_end = k_tile_base + Int32(_NVF4_BLOCK_K)
            if tile_k_end > k_total_rows:
                tile_k_end = k_total_rows
            row_start = Int32(s_k_start[tx])
            row_end = Int32(s_k_end[tx])
        ...
        if s_tile_live[Int32(0)] != Int32(0):
            i = tx
            while i < Int32(_NVF4_BLOCK_K * _NVF4_HEAD_U32):
                ktok = i // Int32(_NVF4_HEAD_U32)
                word = i % Int32(_NVF4_HEAD_U32)
                grow = k_tile_base + ktok
                sK[i] = Uint32(k_e2m1[grow, word]) if grow < k_total_rows else Uint32(0)
                i += Int32(_NVF4_THREADS_PER_CTA)
            i = tx
            while i < Int32(_NVF4_BLOCK_K * _NVF4_NBLK_U32):
                ktok = i // Int32(_NVF4_NBLK_U32)
                word = i % Int32(_NVF4_NBLK_U32)
                grow = k_tile_base + ktok
                sKsf[i] = Uint32(k_sfb[grow, word]) if grow < k_total_rows else Uint32(0)
                i += Int32(_NVF4_THREADS_PER_CTA)
            cute.arch.sync_threads()

            acc_layout = cute.make_layout(
                (1, _NVF4_NUM_MMA_KV, 8), stride=(_NVF4_NUM_MMA_KV * 8, 8, 1)
            )
            lane_group = lane // Int32(4)
            lane_pair_base = Int32(2) * (lane % Int32(4))
            q_local_rs0 = warp_q_idx * Int32(16) + lane_group
            ...
            while head_idx < num_heads:
                ...
                _qk_mma_nvf4(
                    score_frag, q_e2m1, q_sfa, head_idx, q_tile_base, valid_q_rows,
                    sK, sKsf, lane, warp_q_idx, warp_k_idx,
                )
                w_rs0 = (
                    Float32(weights[q_tile_base + q_local_rs0, head_idx])
                    if q_tile_base + q_local_rs0 < valid_q_rows
                    else Float32(0.0)
                )
                w_rs1 = (
                    Float32(weights[q_tile_base + q_local_rs1, head_idx])
                    if q_tile_base + q_local_rs1 < valid_q_rows
                    else Float32(0.0)
                )
                ...
                for mma_kv in cutlass.range_constexpr(_NVF4_NUM_MMA_KV):
                    for reg_id in cutlass.range_constexpr(8):
                        ...
                        if (
                            ...
                            score = Float32(score_frag[0, mma_kv, reg_id] * w_val)
                            if row_slot == Int32(0):
                                local_max_rs0 = attention_ops.fmax(local_max_rs0, score)
                            else:
                                local_max_rs1 = attention_ops.fmax(local_max_rs1, score)
                ...
                cute.arch.sync_threads()
                if tx < Int32(_BLOCK_Q):
                    q_local_out = tx
                    v = s_block_partial[Int32(0), q_local_out]
                    for wk in cutlass.range_constexpr(1, _NVF4_WARPS_K):
                        v = attention_ops.fmax(v, s_block_partial[Int32(wk), q_local_out])
                    q_row_out = q_tile_base + q_local_out
                    if q_row_out < valid_q_rows and k_tile_idx < num_blocks_out:
                        block_scores[head_idx, q_row_out, k_tile_idx] = v
                cute.arch.sync_threads()
                head_idx += Int32(1)
        # dead tile: block_scores pre-filled -inf by the host; nothing to write.


def run_contiguous_block_scores_kernel_nvf4(
    q_e2m1: torch.Tensor,        # uint8 [Qp, H, 64]  e2m1 Q (Qp = padded to _BLOCK_Q)
    q_sfa: torch.Tensor,         # uint8 [Qp, H, 8]   per-16 ue4m3 Q scales
    weights: torch.Tensor,       # f32   [Qp, H]      MSA head weights (== MSA_SM_SCALE)
    k_e2m1: torch.Tensor,        # uint8 [Kr, 64]     contiguous e2m1 K
    k_sfb: torch.Tensor,         # uint8 [Kr, 8]      per-16 ue4m3 K scales
    k_start: torch.Tensor,       # int32 [Qp]
    k_end: torch.Tensor,         # int32 [Qp]
    valid_q_rows: int,
    valid_k_rows: int,
    *,
    num_blocks_out: int | None = None,
    score_mode: int = IndexerScoreMode.MSA_BILINEAR,
    block_scores: torch.Tensor | None = None,
) -> torch.Tensor:
    """Direct native-nvf4 MSA per-head block-score entry (harness/integration use).

    Returns block_scores [num_heads, valid_q_rows, num_blocks_out] (per-head,
    block-max over global 128-token K blocks, causal-masked).  Downstream
    msa_topk_blocks is identical to the fp8 path.
    """
    device = q_e2m1.device
    k_rows = int(k_e2m1.shape[0])
    num_heads = int(q_e2m1.shape[1])
    inferred = (int(valid_k_rows) + _NVF4_BLOCK_K - 1) // _NVF4_BLOCK_K
    if num_blocks_out is None:
        num_blocks_out = inferred
    num_blocks_out = int(num_blocks_out)
    if num_blocks_out < inferred:
        raise ValueError(
            f"num_blocks_out {num_blocks_out} smaller than required {inferred}"
        )
    if block_scores is None:
        block_scores = torch.full(
            (num_heads, valid_q_rows, num_blocks_out),
            float("-inf"),
            dtype=torch.float32,
            device=device,
        )
    else:
        block_scores = block_scores[:num_heads, :valid_q_rows, :num_blocks_out]
        block_scores.fill_(float("-inf"))
    qp = int(q_e2m1.shape[0])
    q_e2m1_u32 = q_e2m1.view(torch.int32).reshape(qp, num_heads, _NVF4_HEAD_U32)
    q_sfa_u32 = q_sfa.view(torch.int32).reshape(qp, num_heads, _NVF4_NBLK_U32)
    k_e2m1_u32 = k_e2m1.view(torch.int32).reshape(k_rows, _NVF4_HEAD_U32)
    k_sfb_u32 = k_sfb.view(torch.int32).reshape(k_rows, _NVF4_NBLK_U32)
    stream = current_cuda_stream()
    weights_c = weights.contiguous()
    k_start_c = k_start.contiguous()
    k_end_c = k_end.contiguous()
    kernel = SparseNSANvf4ContiguousBlockScoresKernel(score_mode=int(score_mode))
    args = (
        _to_kernel_tensor(q_e2m1_u32, cutlass.Uint32, assumed_align=16),
        _to_kernel_tensor(q_sfa_u32, cutlass.Uint32, assumed_align=4),
        _to_kernel_tensor(weights_c, cutlass.Float32, assumed_align=4),
        _to_kernel_tensor(k_e2m1_u32, cutlass.Uint32, assumed_align=16),
        _to_kernel_tensor(k_sfb_u32, cutlass.Uint32, assumed_align=4),
        _to_kernel_tensor(k_start_c, cutlass.Int32, assumed_align=4),
        _to_kernel_tensor(k_end_c, cutlass.Int32, assumed_align=4),
    ...
    # Compile-once: key on FIXED shape/stride facts (fixed-capacity buffers), so
    # valid_q_rows / valid_k_rows / num_blocks / k_start / k_end vary at runtime
    # with NO recompile -- the fp8 supertile compile-once pattern (b12x_launch
    # separates compile facts from runtime args).
    facts = (
        ("variant", "nvf4_msa_blockmax"),
        ("score_mode", int(score_mode)),
        ("head_dim", _INDEX_HEAD_DIM),
        ("q_heads", int(q_e2m1_u32.shape[1])),
        ("q_e2m1_stride_row", int(q_e2m1_u32.stride(0))),
        ("q_e2m1_stride_head", int(q_e2m1_u32.stride(1))),
        ("q_sfa_stride_row", int(q_sfa_u32.stride(0))),
        ("q_sfa_stride_head", int(q_sfa_u32.stride(1))),
        ("weights_stride_q", int(weights_c.stride(0))),
        ("weights_stride_head", int(weights_c.stride(1))),
        ("k_e2m1_stride_row", int(k_e2m1_u32.stride(0))),
        ("k_sfb_stride_row", int(k_sfb_u32.stride(0))),
        ("bs_heads", int(block_scores.shape[0])),
        ("bs_blocks", int(block_scores.shape[2])),
        ("bs_stride_head", int(block_scores.stride(0))),
        ("bs_stride_q", int(block_scores.stride(1))),
        ("bs_stride_block", int(block_scores.stride(2))),
        ("block_k", _NVF4_BLOCK_K),
    )
    compile_spec = KernelCompileSpec.from_facts(
        "attention.indexer.contiguous_block_scores_nvf4", 1, *facts
    )
